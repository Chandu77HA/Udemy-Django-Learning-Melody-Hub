...
def individual_album_list(request, artist_id):
    ...
    get_artist_rating = Album.objects.filter(artist = artist_id).aggregate(Avg('num_stars'))
    album_data = {
...
def edit_artist(request, artist_id):
    ...
    if request.method == 'POST':
        get_artist_form = forms.MusicianForm(request.POST, instance=get_artist_info)

        if get_artist_form.is_valid():
            get_artist_form.save(commit=True)
            artist_data.update({'success_text' : 'Artist Data Successfully Updated !'})
...
def delete_musician(request, artist_id):
    ...
    delete_musician_data = {'delete_success' : 'Musician Deleted Successfully !'}
